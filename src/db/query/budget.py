...
def find_by_name(user_id: str, name: str, limit: int | None = None, offset: int = 0):
    # TODO: более умный поиск
    return select(Budget).where(Budget.user_id == user_id).where(Budget.name == name).limit(limit).offset(offset)


def find_by_text(user_id: str, text: str, *, case_sensitive: bool = False, limit: int | None = None, offset: int = 0):
    # TODO: более умный поиск
