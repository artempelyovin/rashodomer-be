...
async def list_transactions(
    ...
    *,
    ...
    return write_response_list(items=categories, total=total, limit=limit, offset=offset, schema=TransactionSchema)
...
async def update_transaction(
    ...
    transaction_id: Annotated[str, UUID] = Path(..., description="The ID of the category"),
    *,
    user: DetailedUserSchema = Depends(authentication_user),
    transaction_repo: TransactionRepo = Depends(transaction_repo_factory),
    category_repo: CategoryRepo = Depends(category_repo_factory),
    ...
    return write_response(result=category, schema=TransactionSchema)
...
async def delete_transaction(
    ...
    *,
    ...
    manager = TransactionManager(transaction_repo=transaction_repo, category_repo=category_repo)
    category = await manager.delete(user_id=user.id, category_id=transaction_id)
    return write_response(result=category, schema=TransactionSchema)
